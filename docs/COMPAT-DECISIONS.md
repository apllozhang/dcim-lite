...
## D01 业务规则落地（2026-09-11，复评 P1-N4）

**设备移位与供电连接规则：业务确认采用方案 A——有活动供电连接禁止移位。**
...
- 关联未决项：设备**下架**（decommission）与活动连接的关系同样待明确（同族问题），
  现状维持厂商行为（允许下架，连接悬空）；如业务需要可按同模式追加 DEVICE_POWERED。
- 验收测试：`TestMoveWithActiveConnectionBlocked`（接电禁移 → 断开可移 → 复接再禁）。
