...
> 评估对象基线:main `2ed61c5`;本轮交付:PR #37(main `1c32b46`、`2ae621b`)+ PR #38(P1-D 首批,见 §3)。
...
| 复评要求 | 状态 |
|---|---|
| 三级树 | ✅ dualrun.spec:新旧树节点集等价(DC/房间/机柜标签) |
| 设备分页/搜索/状态 | ✅ 新 UI 已有;双跑种子 4 设备行集等价(编码级断言) |
| 三态权限 | ✅ 后端 auth-matrix(既有)+ 新 UI 守卫;旧 UI 注入会话键对照 |
| loading/empty/error/401/超时 | ✅ 组件级 vitest + e2e 401 收口(既有);双跑覆盖数据等价 |
| 新旧双跑与回退演练 | ✅ dualrun.spec(API 请求集合+结构化 DOM+截图)+ P0-R1 回退演练 |
| ALE 视觉 tokens 与基准截图 | ✅ tokens 文件(既有)+ dualrun 四张基准截图(CI artifact) |
| 模块级回退审计 | ✅ flag 切换经遥测通道留服务端痕迹(apiCode=FLAG_SWITCH) |

数据快照来源:**冻结种子**(确定性 API 注入,与差分沙箱同思路)——避免现网数据噪声混入行为差分;种子顺序固定,任何环境可复现。
